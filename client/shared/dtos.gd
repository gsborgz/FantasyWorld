...
class GameServerResponse:
	...
	func _init(_name: String = "", _location: String = "", _url: String = "", _status: Variant = null, _clientsCount: int = 0, _maxClients: int = 0):
		name = _name
		location = _location
		url = _url
		status = _status
	...
	static func from(value: Variant) -> GameServerResponse:
		...
		if typeof(value) == TYPE_DICTIONARY:
			...
			obj.clientsCount = raw.get("clientsCount", 0)
			obj.maxClients = raw.get("maxClients", 0)
			return obj
		return null
